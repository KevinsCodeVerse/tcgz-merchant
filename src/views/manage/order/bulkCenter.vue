<template>
    <div id="">
        <el-tabs v-model="tabStatus" @tab-click="clicktapItem">
            ...
            <el-tab-pane v-for="item in statusList" :label="item.name" :name="item.id.toString()" :key="item.id" v-if="params.type !== '3'"></el-tab-pane>
            <el-tab-pane v-for="item in ptStatusList" :label="item.name" :name="item.id.toString()" :key="item.id" v-if="params.type === '3'"></el-tab-pane>
        </el-tabs>
        <el-form :inline="true" size="medium">
            ...
            <el-form-item>
                <el-select v-model="params.shippingState" placeholder="物流状态" size="small">
                    <el-option v-for="item in shippingStateS" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-table :data="list" v-loading="loading" stripe fixed="right" @selection-change="handleSelectionChange">
            <el-table-column label="勾选" prop="id" align="center" type="selection"> </el-table-column>
        ...
        <!-- 批量发货弹框 -->
        <el-dialog title="批量发货" :visible.sync="deliveryShow" center @close="closeDialog" :close-on-click-modal="false" v-loading="batchDevFlag" width="1500px">
            <el-table :data="orderList" v-loading="loading" stripe style="max-width: 1700px; overflow-x: auto;" :row-height="10" max-height="700px">
                ...
                <el-table-column label="订单号" prop="id" align="center" min-width="180px"> </el-table-column>
                <el-table-column label="快递公司" align="center" min-width="180px">
                    <template slot="header" slot-scope="scope"> 快递公司<span style="color: red">*</span> </template>
                    <template slot-scope="scope">
                        <el-autocomplete
                ...
                <el-table-column label="快递单号" align="center" min-width="180px">
                    <template slot="header" slot-scope="scope"> 快递单号<span style="color: red">*</span> </template>
                    <template slot-scope="scope">
                        <el-input placeholder="请输入快递单号" v-model="scope.row.companyNum" size="mini" clearable> </el-input>
                    </template>
                </el-table-column>
        ...
        <vue-easy-print ref="listPrint">
            <div v-html="returnedHtml" v-for="(item_cm, index_cm) in 2" style="page-break-after:always"></div>
        </vue-easy-print>
    </div>
</template>
<script>
