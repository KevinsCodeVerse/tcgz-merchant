<template>
	<div id="authority_role">
		...
		<el-pagination background :layout="$store.state.isPhone?'total, prev, pager, next':'total, sizes, prev, pager, next, jumper'" :page-size="requestParam.pageSize" :total="total"
	...
	export default {
		...
		methods: {
			...
			del(id, accountNum) {
				...
				this.$confirm('此操作将永久删除该角色和改角色下的所有账号, 是否继续?', '提示', {
					...
					this.$request.post({
						...
						success: result => {
							...
							if (this.requestParam.pageNo > 1 && this.list.length < 2) {
