<template>
    <div id="">
        <el-form :inline="true" size="medium" class="group">
            <el-form-item>
        ...
        <el-table :data="list" v-loading="loading" stripe fixed="right">
            ...
            <el-table-column label="产品主图" align="center">
                <template slot-scope="scope">
                    <el-image fit="cover" style="width: 100px; height: 100px" :src="scope.row.avatar | fullPath" :preview-src-list="[$fullPath(scope.row.avatar)]"> </el-image>
                </template>
            </el-table-column>
            <el-table-column label="普通商品原价" align="center">
                <template slot-scope="scope">
                    <span class="warning">￥{{ scope.row.proPrice }}</span>
                </template>
            </el-table-column>
            <el-table-column label="团购商品价格" align="center">
                <template slot-scope="scope">
            ...
            <!-- <el-table-column align="center" label="结束时间">
                <template slot-scope="scope">
                    <el-progress :text-inside="true" :stroke-width="24" :percentage="progressFunc(scope.row)" status="success"></el-progress>
                </template>
            </el-table-column> -->

            <el-table-column align="center" label="团购时间">
                <template slot-scope="scope">
                    <p>{{ $common.getDate(scope.row.openGroupTime) }}</p>
                    <b class="warning">至</b>
                    <p>{{ $common.getDate(scope.row.overGroupTime) }}</p>
                </template>
            </el-table-column>
             <el-table-column label="标签" align="center" width="80px">
                <template slot-scope="scope">
...
export default {
    ...
    methods: {
        ...
        isStatus(row) {
            ...
            return nowTime > row.overGroupTime || row.status == 0;
        },
        // 进度条
        progressFunc(row) {
            let num = row.overGroupTime - row.openGroupTime;
            let now = Date.now() - row.openGroupTime;
            if (now < 1) {
                return 0;
            }
            let progress = Number(((now / num) * 100).toFixed(2));
            console.log(progress);
            return progress > -1 && progress < 100 ? progress : 100;
        },
        // 活动状态管理
        statusSet(row) {
            this.$request.post({
                url: '/mt/group/pro/updateGroupProStatus',
                params: {
                    id: row.id,
        ...
        // 编辑
        update(id) {
            this.$router.push(`/group/group_edit?id=${id}&type=3`);
        },
        // 删除
        remove(id) {
...
.group {
    ...
    padding-left: 15px;
    margin-bottom: 15px;
    background-color: rgba(0, 131, 0, 0.089);
    border-radius: 3px;
}
.btnFalse {
    cursor: no-drop;
